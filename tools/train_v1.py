...
sys.path.insert(0, '/home/suman/Desktop/sphoc-pytorch')
import copy
...
def train():
    ...
    parser.add_argument('--test_interval', action='store', type=int, default=2000,
                        help='The number of iterations after which to periodically evaluate the SPHOC. Default: 500')
    ...
    parser.add_argument('--phoc_unigram_levels', '-pul',
                        ...
                        help='The comma seperated list of SPHOC unigram levels. Default: 1,2,4,8')
    ...
    parser.add_argument('--decoder', '-dec', dest='decoder', default=True, action='store', type=str2bool, help='Whether decoder is trained or not')
    parser.add_argument('--len', '-len', dest='len', default=True, action='store', type=str2bool, help='Whether word length is estimated or not')
    parser.add_argument('--pos', '-pos', dest='pos', default=True, action='store', type=str2bool, help='Whether position encoding is used or not')
    ...
    if args.dataset == 'iam':
        ...
        label_encoder = test_set.label_encoder
    ...
    sphoc_model = SPHOC(enocder_type='phocnet', decoder=args.decoder, length_embedding=args.len, position_embedding=args.pos)
    fmap = open('map_sphoc_new.txt', 'w')
    ...
    for iter_idx in range(max_iters):
        if iter_idx % args.test_interval == 0: # and iter_idx > 0:
            ...
            loss_test, mAp, acc, acc_char = test.evaluate_cnn_batch(cnn=sphoc_model,
                               dataset_loader=test_loader,
                               args=args, loss_fn=(phoc_loss, phoc_loss, char_loss),
                               label_encodings=label_encoder)
            logger.info('mAP after %d iterations: %3.2f %f %f %f', iter_idx, mAp, acc, acc_char, loss_test)

            fmap.write('mAP after '+ str(iter_idx)+' iterations:'+str(mAp)+', loss: '+str(loss_test)+'\n')
            # fmap.close()
            if mAp > best_map:
                best_map = mAp
                torch.save(sphoc_model, 'SPHOC_len_char.pt')
        # if args.gpu_id is not None:
        #     if len(args.gpu_id) > 1:
        #         sphoc_model = nn.DataParallel(sphoc_model, device_ids=args.gpu_id)
        #         sphoc_model.cuda()
        ...
        for _ in range(args.iter_size):
            ...
            try: 
                word_img, embedding, embedding_char, len_embed, _, _  = next(train_loader_iter)
                #print('success')
            ...
            if args.gpu_id is not None:
                if len(args.gpu_id) > 1:
                    ...
                    embedding_char = embedding_char.cuda()
            ...
            # print(out_keys)
            fraction = 1.0/len(out_keys)
            loss_val_total = 0.0
            for key in out_keys:
                if key == 'phoc':
                    output = retval[key]
                    loss_val = phoc_loss(output, embedding) * args.batch_size
                    loss_val_total = fraction * loss_val
                    # loss = loss + loss_val.cpu().item()
                ...
                if key == 'length_vec':
                    ...
                    loss_val_total = loss_val_total + fraction * loss_len
                    # loss = loss + loss_len.cpu().item()
                ...
                if key == 'char_vec':
                    ...
                    loss_char = char_loss(F.softmax(out_char_embedding.contiguous().view(-1, 37), dim=1), embedding_char.view(-1)) * args.batch_size
                    loss_val_total = loss_val_total + fraction * loss_char
            # if len(out_keys) ==3:
            #     loss_val_total = 0.33 * loss_val + 0.33 * loss_len + 0.33 * loss_char
            # elif len(out_keys) ==2:
            ...
            #     loss_val_total = loss_val
                    ...
                    # loss = loss + loss_char.cpu().item()
                    # out_chars = torch.softmax(out_char_embedding.contiguous().view(-1, 37), dim=1).view(8, 10, 37)
                    # out_chars_embedding[lc] = out_chars.view(8, -1).data.cpu().numpy()
            ...
            #     loss_len = phoc_loss(out_length, len_embed) * args.batch_size
            #     loss_char = char_loss(pred_chars, embedding_char) * args.batch_size
            #     # hard coded weighting function between loss
            #     loss_val_total = 0.33 * loss_val + 0.33 * loss_len + loss_char
            # elif len(retval) == 2:
        ...
        if (iter_idx+1) % args.display == 0:
            msg_text = 'Iteration ' + str(iter_idx+1) + ' '+str(loss_val.item()) + ' '
            if sphoc_model.length_embedding:
                msg_text = msg_text + str(loss_len.item()) + ' '
            if sphoc_model.decoder:
                msg_text = msg_text + str(loss_char.item())

            logger.info(msg=msg_text)
            # else:
            #     logger.info('Iteration %*d: %f %f', len(str(max_iters)), iter_idx + 1, loss_val.item(), loss_char.item())
